...
class DiscordRPC extends EventEmitter {
  ...
  async _connectLoop() {
    ...
    if (!clientId) { this.emit('attempt-failed'); return this._scheduleRetry(15000); }
    ...
    this.connecting = false;
    this.emit('attempt-failed'); // Discord kapalı olabilir; sessizce yeniden dene
    this._scheduleRetry(20000);
  }
  ...
  _tryConnect(pipePath, clientId) {
    return new Promise((resolve) => {
      ...
      sock.setTimeout(1000, () => { sock.destroy(); finish(false); });
